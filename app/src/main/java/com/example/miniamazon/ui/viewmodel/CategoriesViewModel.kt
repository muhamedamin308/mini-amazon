...
class CategoriesViewModel(
    private val fireFireStore: FirebaseFirestore, private val category: Category
) : ViewModel() {
    private val mOfferProducts = MutableStateFlow<Status<List<Product>>>(Status.UnSpecified())
    val offerProducts = mOfferProducts.asStateFlow()

    private val mAllProducts = MutableStateFlow<Status<List<Product>>>(Status.UnSpecified())
    ...
    private fun fetchOfferProducts() {
        ...
        fireFireStore.collection(Constants.Collections.PRODUCTS_COLLECTION)
            .whereEqualTo(Constants.Collections.CATEGORY_COLLECTION, category.category)
            .whereNotEqualTo(IS_OFFER_EXIST, null).get().addOnSuccessListener {
    ...
    private fun fetchAllProducts() {
        ...
        fireFireStore.collection(Constants.Collections.PRODUCTS_COLLECTION)
            .whereEqualTo(Constants.Collections.CATEGORY_COLLECTION, category.category)
            .whereEqualTo(IS_OFFER_EXIST, null).get().addOnSuccessListener {
                val products = it.toObjects(Product::class.java)
                viewModelScope.launch {
